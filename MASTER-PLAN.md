...
- [ ] **T1.3** Product gallery: grid, filters (category, availability, price, newest), SOLD badge, sold items sorted last. GATE: with seeded data, every filter visibly changes results; a sold item shows the badge and sorts last.
- [ ] **T1.4** Product details page: photo carousel, all §3 fields, Copy Order Message, `ig.me` button, disabled **Sold** state, slug URL. GATE: copy button works on a real phone; a sold product shows the disabled button.
- [ ] **T1.5** Content pages: How to Order, Shipping, About, Contact, Policies. GATE: all reachable from the nav and clean on mobile.
- [ ] **T1.6** SEO layer per §10: per-page metadata + canonical tags, OG/Twitter cards, JSON-LD (Product with availability, Organization, BreadcrumbList), `sitemap.xml` generated from the database, `robots.txt`, `noindex` on `/admin`. GATE: Google Rich Results Test passes on a product URL; the sitemap lists every published product; robots blocks `/admin`.
- [ ] **T1.7** On-demand revalidation wired to every admin save. GATE: marking an item Sold Out in the admin updates the public product page and gallery within 10 seconds, no redeploy.
...
- [ ] **T2.1** Orders: compact table, §5 status pipeline, items linked to products, centavo-exact totals with shipping and discount. GATE: an order with two items matches hand-computed totals exactly.
- [ ] **T2.2** Invoices: one click from an order, sequential numbering. GATE: totals match the order; three consecutive invoices number without gaps or duplicates.
- [ ] **T2.3** Expenses: quick-add with three visible fields (amount, category, date defaulting to today) and an expandable full form; optional product/order links. GATE: a quick-add on a phone takes ≤ 15 seconds and appears in reports instantly.
- [ ] **T2.3b** Enter opening expense #1 from §5: 2026-08-05 · Website and domain · Cloudflare · torisabi.com domain registration (1 yr) · ₱731.01 · card. GATE: the row stores exactly 73101 centavos and appears in the August 2026 profit report.
- [ ] **T2.4** Profit reports per §5: presets (today, this week, this month, last month) + custom range; sales, expenses by category, profit, per-item margin, monthly trend. GATE: a seeded dataset with pre-computed expected numbers matches to the centavo.
- [ ] **T2.5** Dashboard stat tiles wired to live data. GATE: tiles equal the report values for the same period.
