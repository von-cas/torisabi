...
**Build Status (living):** `LIVE at https://www.torisabi.com` — Phase 0 done except T0.5; Phase 1 built, public-facing gates passed; Phase 2 (orders, invoices, expenses, reports) built and arithmetic-verified. **Blocked on Von:** T0.5 — sign in once and enrol an authenticator at `/admin/security`. Until then no admin screen can load data (migration 0003), so every admin gate stays unchecked. · Last updated: 2026-08-05
...
- [ ] **T0.5** Auth: admin account(s) created, public signups disabled, TOTP MFA enrolled. GATE: a signup attempt is rejected; login with a wrong TOTP code fails.
  — 2026-08-05: **public signups disabled** and **TOTP enabled** (both done in the Supabase dashboard; AAL1 session limiting is on). One shared account created by Von for his wife. **Remaining and blocking:** that account must sign in once and enrol an authenticator at `/admin/security` — Supabase has no dashboard UI for enrolment, so it can only happen in the app. Until it does, migration 0003 correctly returns zero rows to every admin screen.
...
- [ ] **T1.15** `/api/health` (trivial DB select) + daily **Hermes** cron keep-alive (not a Cloudflare Cron Trigger — see §7). GATE: endpoint returns 200 with a DB round-trip; the Hermes cron has run at least once against the live URL.
  — 2026-08-05: endpoint **done and live** — `https://www.torisabi.com/api/health` returns `{"ok":true,"db":"up","checked_at":"…"}`, proving the worker reaches Supabase from Cloudflare. Remaining: register the daily Hermes cron that curls it.
