...
- [ ] **T12.8** **Auto-deploy from GitHub is connected but blocked.** Netlify's free Starter plan refuses to build a **private** repo unless the Git contributor is a verified account member, and Von's Netlify account is linked via **Google only — no GitHub identity**, so `von-cas` is unrecognised. Error: `Build blocked: Unrecognized Git contributor.` Ruled out the `Co-Authored-By` trailer as the cause by pushing a commit without one — still blocked.
  **Fix (needs Von, ~1 min):** Netlify → user settings → **Connected accounts → connect GitHub**. Then pushes to `main` build automatically.
  Alternatives if that fails: make the repo public (the restriction only applies to private repos; no secrets live in the repo), or keep deploying with `npx netlify deploy --build --prod`, which works today.
  GATE: a push to `main` produces a `ready` deploy with no manual step.
